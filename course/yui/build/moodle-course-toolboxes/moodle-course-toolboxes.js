...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    toggle_hide_section: function(e) {
        ...
        // BPM - Dialog and notification for students
        if (data.value == 1) {
            this.bpm_should_send_student_notice(data);
        };
    },
    ...
    bpm_should_send_student_notice: function(data) {
        var revealDialogElement = $('<div id="revealDialogElement"/>');

        var options = {
            autoOpen:  false,
            draggable: false,
            modal:     true,
            title:     'התראה לסטודנטים',
            closeOnEscape: false,
            classes: {
                "ui-dialog": "ui-dialog ui-corner-all ui-widget ui-widget-content ui-front ui-dialog-buttons ui-resizable tn-dialog"
            },
            open: function() {
                $(this).html("האם לשלוח התראה לסטודנטים על חשיפת התוכן?");
            },
            buttons: [{
                text: "כן",
                click: function() {
                    ...
                    // Send an email notification to students about the revealed course section
                    $.post('https://my.bpm-music.com/blocks/bpm_utils/student_content_notice.php', 
                           { sectionId: data.id, courseId: data.courseId }).done(function() {
                        alert('נשלחה התראה לסטודנטים');
                    });
                }
            },{ 
                text: "לא",
        ...
        revealDialogElement.dialog(options).dialog('open');
    }
