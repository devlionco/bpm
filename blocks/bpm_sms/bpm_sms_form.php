...
class bpm_sms_form extends moodleform {
    ...
    public function display_report() {
        ...
        $table->head  = array(get_string('serial_no', 'block_bpm_sms'), get_string('name', 'block_bpm_sms'), get_string('cell_no', 'block_bpm_sms'), get_string('select', 'block_bpm_sms'));
...
class bpm_sms_send extends moodleform {
    public function definition() {
        global $DB, $CFG;
        $mform =& $this->_form;
        $mform->addElement('header', 'bpm_sms_send', get_string('bpm_sms_send', 'block_bpm_sms'));
        if(isset($c_id)) {
	...
	$course_id  = required_param('c_id',PARAM_INT);
        $courses_select = $mform->addElement('select', 'c_id', get_string('selectcourse', 'block_bpm_sms'), $attributes);
	$courses_select->setSelected($course_id);
        $mform->setType('c_id', PARAM_INT);
	if(isset($c_id)) {
	    $attributes =  $DB->get_records_sql_menu('SELECT id,level_name FROM {competency_level} where id = ?',array ($c_id), $limitfrom=0, $limitnum=0);
        }
        else {
        ...
        $mform->addElement('select', 'r_id', get_string('selectrole', 'block_bpm_sms'), $attributes);
        $attributes =  $DB->get_records_sql_menu('SELECT id,tname FROM {block_bpm_sms_template}', null, $limitfrom=0, $limitnum=0);
        $mform->addElement('selectwithlink', 'm_id', get_string('selectmsg', 'block_bpm_sms'), $attributes, null,
                           array('link' => $CFG->wwwroot.'/blocks/bpm_sms/view.php?viewpage=3', 'label' => get_string('template', 'block_bpm_sms')));
        $attributes = array('rows' => '6', 'cols' => '45', 'maxlength' => '160');
        $mform->setType('r_id', PARAM_INT);
        $mform->addElement('textarea', 'bpm_sms_body', get_string('bpm_sms_body', 'block_bpm_sms'), $attributes);
        $mform->addRule('bpm_sms_body','Please write Message' , 'required', 'client');
        $mform->addRule('bpm_sms_body', $errors = null, 'required', null, 'server');
        $mform->setType('bpm_sms_body', PARAM_TEXT);
...
$sql="SELECT usr.firstname, usr.id, usr.lastname, usr.email,usr.phone2,c.fullname
            ...
            INNER JOIN {user_enrolments} ue ON e.id = ue.enrolid AND ue.userid = usr.id
            WHERE r.id = $r_id AND ue.status != 1";
        ...
        if($count >= 1) {
            ...
            foreach ($rs as $log) {
                ...
                $row[] = $log->firstname;
                $row[] = $log->phone2;
                $row[] = "<input type='checkbox' class='usercheckbox' checked name='user[]' value='$log->id'/>";
                $table->data[] = $row;
            }
        }
        else {
...
class template_form extends moodleform {
    public function definition() {
        $mform =& $this->_form;
        $mform->addElement('header', 'bpm_sms_template_header', get_string('bpm_sms_template_header', 'block_bpm_sms'));
  ...
  public function validation($data, $files) {
        ...
        if ($data['tname'] == "") {
            ...
            if ($DB->record_exists('block_bpm_sms_template', array('tname' => $data['tname']))) {
    ...
    public function display_report() {
        global $DB, $OUTPUT, $CFG, $USER;
        $table = new html_table();
        $table->head  = array(get_string('serial_no', 'block_bpm_sms'), get_string('name', 'block_bpm_sms'), get_string('msg_body', 'block_bpm_sms'), get_string('edit', 'block_bpm_sms'), get_string('delete', 'block_bpm_sms'));
        $table->size  = array('10%', '20%', '50%', '10%', '10%');
        $table->align  = array('center', 'left', 'left', 'center', 'center');
        $table->width = '100%';
        $table->data  = array();
        $sql="SELECT * FROM {block_bpm_sms_template}";
        $rs = $DB->get_recordset_sql($sql, array(),  null, null);
        ...
        foreach ($rs as $log) {
            ...
            $row[] = $log->tname;
            $row[] = $log->template;
            $row[] = '<a  title="Edit" href="'.$CFG->wwwroot.'/blocks/bpm_sms/view.php?viewpage=3&edit=edit&id='.$log->id.'"/><img src="'.$OUTPUT->pix_url('t/edit') . '" class="iconsmall" /></a> ';
            $row[] = '<a  title="Remove" href="'.$CFG->wwwroot.'/blocks/bpm_sms/view.php?viewpage=3&rem=remove&id='.$log->id.'"/><img src="'.$OUTPUT->pix_url('t/delete') . '" class="iconsmall"/></a>';
            $table->data[] = $row;
        }
        return $table;
    }
