...
class sendbpm_smsdotpk {
	
    // Private Variables for API's use.
    private $apikey;
    private $apiUrl = "http://api.sendbpm_sms.pk/";

    // Public Functions for APP's use.
    // Deafult Constructor.
    function sendbpm_smsdotpk($ak){
        $this->apikey = $ak;
    }
    ...
    function sendbpm_sms($phone, $msg, $type = 0){
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->api_url("sendbpm_sms"));
