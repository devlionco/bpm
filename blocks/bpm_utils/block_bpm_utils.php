...
class block_bpm_utils extends block_base {
    ...
    public function get_content() {
        ...
        if ($this->bpm_check_ssd_capabilities($USER->id)) {
            ...
            $url = (strpos($COURSE->shortname, "אונליין") > -1) ?
                "https://docs.google.com/presentation/d/1X1twC_03htxWSAY5qRzU8hRdUaxba3a7WYZG99b5WSo/present" :
                "https://docs.google.com/presentation/d/1bEsoofQ3sS_0tpOz7vdYaAQ3yV3GUj1HdNc6in_hZPg/present";
            $html = "<a class=\"bpm_utils_links\" href=" . $url .
        ...
        //if ($USER->id == 3 || $USER->id == 4058 || $USER->id == 363) {
        ...
        if (!$this->is_bpm_staff($USER->id)) {
            ...
            if (strpos($branch_names_string, "אונליין") > -1) {
                $online = true;
                echo "<script>document.addEventListener('DOMContentLoaded', function() {
                    $('a[title=\'הרשמה לתרגול קבוצתי\']').remove();
                    }, false);</script>";
            }
    ...
    public function bpm_check_for_es_course_reg_and_completion($user_id) {
        ...
        echo '<script>console.log(' . json_encode($electric_course_id) . ');</script>';
        echo '<script>console.log("ay");</script>';
        if ($electric_session_count > 0) {
            return true;
        } else if ($electric_course_id) {
...
//  echo "<script>console.log('" . $course_id . "');</script>";
    ...
    public function bpm_get_makeup_test_url($course_id) {
        ...
        $acuity_match_sql = "SELECT appointment_type_id, name FROM mdl_bpm_acuity_appointments 
                            ...
                            OR LOCATE(BINARY ' $course_info->parentid ', CONCAT(' ', course_parent_ids, ' '))
                            OR LOCATE(BINARY ',$course_info->parentid ', CONCAT(' ', course_parent_ids, ' ')))
                            AND (branch LIKE '$course_info->branch' OR ('$course_info->branch' = NULL) OR '$course_info->branch' = 'אונליין')";
